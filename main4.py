...
def meklēt_uzn_info():
    def atrast_uzn_info():
        nosaukums= nosaukums_entry.get()
        if nosaukums:
            cursor.execute("SELECT nosaukums, adrese,e_pasts, tel_num, Pilsetas.pilsetas_nos,logo_veids FROM Uznemums JOIN Pilsetas ON Uznemums.id_pilsetas=Pilsetas.id_pilsetas JOIN Logo ON Uznemums.id_logo=Logo.id_logo WHERE Uznemums.nosaukums LIKE ?", (f"%{nosaukums}%",))
            rezultati = cursor.fetchall()
